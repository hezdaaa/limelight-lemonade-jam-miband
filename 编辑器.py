...
class GalGameScriptEditor:
    ...
    def create_dialogue_list_column(self, parent):
        ...
        ttk.Button(button_frame, text="刷新列表", command=self.update_dialogue_list_lazy, width=10).pack(side="left", padx=2)
        ttk.Button(button_frame, text="跳转到选中", command=self.jump_to_selected_dialogue, width=12).pack(side="left", padx=2)
        ttk.Button(button_frame, text="删除选中", command=self.delete_selected_dialogues, width=10).pack(side="right", padx=2)
        self.current_page = 1
        self.total_pages = 1
        self.filtered_dialogues = []
    ...
    def create_enhanced_preview_area(self, parent):
        ...
        ttk.Button(preview_header, text="刷新预览", command=self.update_realtime_preview, width=10).pack(side="right", padx=5)
    ...
    def _create_basic_field_form(self, parent):
        ...
        ttk.Button(template_input_frame, text="保存模板", command=self.save_xyz_template, width=10).pack(side="right", padx=2)
    ...
    def on_co_changed(self):
        """根据‘是否有选项’复选框的状态启用或禁用选项输入框"""
        state = 'normal' if self.co_var.get() else 'disabled'
        for entry in [self.c1_entry, self.c2_entry, self.c3_entry, self.c4_entry,
                      self.c1t_entry, self.c2t_entry, self.c3t_entry, self.c4t_entry]:
    ...
    def apply_to_json(self):
        ...
        try:
            ...
            if self.co_var.get():
                ...
                for key in ['co', 'c1', 'c1t', 'c2', 'c2t', 'c3', 'c3t', 'c4', 'c4t']:
                    script.pop(key, None)
            ...
            def load_and_composite_images():
                try:
                    ...
                    if not (bg_loaded or char_loaded or cg_loaded):
                        ...
                        draw.multiline_text((168, 280), settings_text, fill=(136, 136, 136, 255), font=None, anchor="mm", align="center", spacing=5)
                    self.root.after(0, lambda: self.display_composite_image(composite_image, bg_loaded, char_loaded, cg_loaded))
                except Exception as e:
                    print(f"合成图像失败: {e}")
    ...
    def insert_dialogue_before_current(self):
        ...
        self.update_dialogue_list_lazy()
        if messagebox.askyesno("保存确认", f"插入操作已完成，移动了 {moved_count} 条对话，影响了 {len(affected_chunks)} 个块文件。\n是否立即保存这些更改到文件？"):
        ...
        messagebox.showinfo("插入成功", f"已在位置 {insert_pos} 插入新对话\n移动了 {moved_count} 条对话\n影响了 {len(affected_chunks)} 个块文件")
    def save_affected_chunks_after_insert(self, affected_chunks):
        ...
        try:
            ...
            if failed_chunks:
                ...
                messagebox.showwarning("保存警告", f"成功保存了 {saved_count} 个块文件\n以下 {len(failed_chunks)} 个块保存失败: {failed_chunks}")
            else:
                self.status_label.config(text=f"已保存 {saved_count} 个受影响的块文件 {time.strftime('%H:%M:%S')}", foreground="#6a9955")
        except Exception as e:
    ...
    def delete_dialogue(self):
        ...
        self.sorted_ids = None
        self.update_dialogue_list_lazy()
        if messagebox.askyesno("保存确认", f"删除操作已完成，移动了 {moved_count} 条对话，影响了 {len(affected_chunks)} 个块文件。\n是否立即保存这些更改到文件？"):
            self.save_affected_chunks_after_insert(affected_chunks)
    ...
    def save_all_changes(self):
        ...
        try:
            ...
            if failed_chunks:
                self.status_label.config(text=f"部分保存失败", foreground="#ff6666")
                messagebox.showwarning("保存警告", f"成功保存了 {total_chunks - len(failed_chunks)}/{total_chunks} 个文件\n以下 {len(failed_chunks)} 个块保存失败: {failed_chunks}")
            else:
    ...
    def import_script(self):
        file_path = filedialog.askopenfilename(filetypes=[("JSON文件", "*.json"), ("文本文件", "*.txt"), ("所有文件", "*.*")])
        if file_path:
            try:
