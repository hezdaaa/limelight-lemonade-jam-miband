...
class GalGameScriptEditor:
    ...
    def setup_dark_theme(self):
        ...
        style.map('TNotebook.Tab',
                 background=[('selected', '#1e1e1e'), ('active', '#555555')],
    ...
    def create_menu(self):
        """创建菜单栏"""
        menubar = tk.Menu(self.root, bg='#252526', fg='#d4d4d4', activebackground='#555555', activeforeground='#ffffff')
        self.root.config(menu=menubar)
        
        file_menu = tk.Menu(menubar, tearoff=0, bg='#252526', fg='#d4d4d4', activebackground='#555555', activeforeground='#ffffff')
        ...
        edit_menu = tk.Menu(menubar, tearoff=0, bg='#252526', fg='#d4d4d4', activebackground='#555555', activeforeground='#ffffff')
        ...
        tool_menu = tk.Menu(menubar, tearoff=0, bg='#252526', fg='#d4d4d4', activebackground='#555555', activeforeground='#ffffff')
    ...
    def create_widgets(self):
        ...
        self.progress_spinbox = ttk.Spinbox(
            nav_frame,
            from_=1,
            to=1000000,
        ...
        ttk.Label(nav_frame, text="快捷键: Ctrl+A/D=上下对话, Ctrl+S=保存, Ctrl+Z/Y=撤销重做, Ctrl+P=复制立绘CG").pack(side="left", padx=20)
        ...
        ttk.Button(button_frame, text="复制上一页 (Ctrl+P)", command=self.copy_previous_character_and_cg).pack(side="left", padx=5)
    ...
    def create_dialogue_list_column(self, parent):
        ...
        ttk.Button(pagination_frame, text="<<", width=2, 
                  command=lambda: self.change_page(-1)).pack(side="left", padx=1)
        ...
        self.dialogue_listbox = tk.Listbox(
            list_frame,
        ...
        ttk.Button(button_frame, text="刷新列表", command=self.update_dialogue_list_lazy,
                  width=10).pack(side="left", padx=2)
        ttk.Button(button_frame, text="跳转到选中", command=self.jump_to_selected_dialogue,
                  width=12).pack(side="left", padx=2)
        ttk.Button(button_frame, text="删除选中", command=self.delete_selected_dialogues,
    ...
    def batch_fill_background_no_overwrite(self):
        """批量填充背景（不覆盖已有字段）"""
        self._batch_fill_resource_from_selection(self.bcgi_path, 'b', 
                                                 ['.jpg', '.jpeg', '.png', '.bmp', '.gif'],
                                                 "背景", 'last_selected_background',
                                                 overwrite=False)
    ...
    def batch_fill_cg_no_overwrite(self):
        """批量填充CG（不覆盖已有字段）"""
        self._batch_fill_resource_from_selection(self.evig_path, 'cg',
    ...
    def create_quick_commands(self, parent):
        ...
        ttk.Label(parent, text="点击立绘名称可快速应用到当前对话", font=("微软雅黑", 8), foreground="#858585").pack(anchor="w", padx=5, pady=(0, 5))
    ...
    def create_enhanced_preview_area(self, parent):
        ...
        ttk.Button(preview_header, text="刷新预览", command=self.update_realtime_preview,
                  width=10).pack(side="right", padx=5)
        
        self.composite_canvas = tk.Canvas(parent, bg="#0d0d0d", width=336, height=480, highlightthickness=1, highlightbackground="#555555")
        self.composite_canvas.pack()
        
        self.composite_canvas.create_text(168, 240, text="预览区域\n图像将在这里叠加显示", 
                                         fill="#555555", font=("微软雅黑", 11), justify="center")
        
        self.xyz_display_label = ttk.Label(parent, text="缩放模式: 0 (正常大小)", 
                                          font=("微软雅黑", 9))
        ...
        ttk.Label(explanation_frame, text="中层: 立绘 (cimg文件夹) - 受缩放影响", font=("微软雅黑", 8), foreground="#858585").pack(anchor="w")
    ...
    def create_field_form(self, parent):
        ...
        self._create_field_form_content(scrollable_frame)
        
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
    def _create_field_form_content(self, parent):
        """创建字段表单内容"""
        row = 0
        ...
        ttk.Button(button_frame, text="解析立绘名", command=self.parse_character_name,
                  width=12).pack(side="left", padx=2)
        ttk.Button(button_frame, text="拼接立绘名", command=self.generate_character_name,
                  width=12).pack(side="left", padx=2)
        row += 1
        ...
        self.z_var = tk.IntVar(value=0)  # 0=无缩放，1=小，2=大
        ...
        ttk.Button(template_input_frame, text="保存模板", command=self.save_xyz_template,
                  width=10).pack(side="right", padx=2)
        ...
        ttk.Button(button_frame, text="应用到JSON", command=self.apply_to_json, 
                  width=15).pack(side="left", padx=5)
        ttk.Button(button_frame, text="从JSON加载", command=self.load_from_json,
                  width=15).pack(side="left", padx=5)
        ttk.Button(button_frame, text="清除所有", command=self.clear_fields,
                  width=15).pack(side="left", padx=5)
    ...
    def _select_resource(self, resource_type, resource_path, extensions, memory_attr):
        ...
        select_window.wait_window()  # 等待窗口关闭
        return result
    ...
    def choose_cg(self):
        """选择CG"""
        selected = self._select_resource("CG", self.evig_path,
                                         ['.png', '.jpg', '.jpeg', '.gif', '.bmp'], 
                                         'last_selected_cg')
        if selected:
    ...
    def update_template_buttons(self):
        ...
        for template_name in self.xyz_templates.keys():
            ...
            apply_btn = ttk.Button(
                btn_frame,
                text=template_name,
                command=lambda name=template_name: self.apply_template_button(name),
                width=12
            )
            ...
            delete_btn = ttk.Button(
                btn_frame,
                text="×",
                command=lambda name=template_name: self.delete_template_button(name),
                width=3
            )
    ...
    def apply_template_button(self, template_name):
        ...
        if template_name in self.xyz_templates:
            ...
            self.z_var.set(template['z'])  # 直接设置模式值
    ...
    def save_xyz_template(self):
        ...
        self.xyz_templates[template_name] = {
            'z': self.z_var.get()  # 存储模式值（0,1,2）
        }
    ...
    def copy_previous_zoom(self):
        ...
        if prev_key in self.script_data:
            ...
            elif abs(z_value - 0.8) < 0.1:   # 兼容旧数据
                self.z_var.set(1)
    ...
    def analyze_character_names(self):
        ...
        try:
            char_files = [f for f in os.listdir(self.cimg_path) 
        ...
        try:
            ...
            if progress_key in self.script_data:
                ...
                if z_value is None:
                    ...
                    self.z_var.set(z_value)
                elif abs(z_value - 0.8) < 0.1:   # 兼容旧数据（小）
                    self.z_var.set(1)
                elif abs(z_value - 1.2) < 0.1:   # 兼容旧数据（大）
                    self.z_var.set(2)
                else:
                    self.z_var.set(0)
    ...
    def update_realtime_preview(self):
        ...
        try:
            ...
            self.xyz_display_label.config(text=f"缩放模式: {zoom_mode} ({z_value}x)" if zoom_mode>0 else f"缩放模式: 0 (正常大小)")
            ...
            def load_and_composite_images():
                try:
                    ...
                    if not (bg_loaded or char_loaded or cg_loaded):
                        ...
                        draw.text((168, 200), "无图像预览", fill=(136, 136, 136, 255), 
                                 font=None, anchor="mm")
                        
                        settings_text = f"背景: {bg_text or '无'}\n立绘: {char_text or '无'}\nCG: {cg_text or '无'}\n缩放: {zoom_mode}({z_value}x)" if zoom_mode>0 else f"背景: {bg_text or '无'}\n立绘: {char_text or '无'}\nCG: {cg_text or '无'}\n缩放: 无缩放"
                        draw.multiline_text((168, 280), settings_text, fill=(136, 136, 136, 255), 
                                          font=None, anchor="mm", align="center", spacing=5)
                    
                    self.root.after(0, lambda: self.display_composite_image(composite_image, 
                                                                           bg_loaded, char_loaded, cg_loaded))
    ...
    def display_composite_image(self, composite_image, bg_loaded, char_loaded, cg_loaded):
        try:
            ...
            status_parts = []
            if bg_loaded:
                status_parts.append("背景")
            if char_loaded:
                status_parts.append("立绘")
            if cg_loaded:
                status_parts.append("CG")
    ...
    def display_error(self):
        self.composite_canvas.delete("all")
        self.composite_canvas.create_text(168, 240, text="图像加载失败", 
    ...
    def insert_dialogue_before_current(self):
        ...
        messagebox.showinfo("插入成功", 
                           ...
                           f"移动了 {moved_count} 条对话\n"
                           f"影响了 {len(affected_chunks)} 个块文件")
        ...
        try:
            ...
            if failed_chunks:
                ...
                messagebox.showwarning("保存警告", 
                                     ...
                                     f"以下 {len(failed_chunks)} 个块保存失败: {failed_chunks}")
            else:
    ...
    def optimize_storage(self):
        ...
        messagebox.showinfo("优化完成", 
                           f"储存优化完成！\n"
                           f"优化前: {before_count} 个对话\n"
                           f"优化后: {after_count} 个对话\n"
                           f"删除了 {deleted_empty_dialogues} 个空对话")
    ...
    def update_dialogue_list_lazy(self):
        ...
        if search_term:
            ...
            for d_id in dialogue_ids:
                ...
                if (search_term in str(d_id).lower() or 
                    search_term in speaker.lower() or 
                    search_term in text_preview.lower()):
