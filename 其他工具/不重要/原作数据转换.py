...
class GalgameScriptConverter:
    ...
    def export_txt(self):
        ...
        try:
            ...
            # 如果 JSON 解析失败，给出详细错误信息并询问是否仍要导出为纯文本
    ...
    def find_all_script_entries_ordered(self, input_data):
        # 1. 扫描所有 chapter 匹配（大小写不敏感），提取数字-数字结构
        ...
        all_items = []  # 存储最终条目，每个元素为 (position, entry_dict, is_dialogue)
    ...
    def convert_script_thread(self):
        try:
            ...
            # 构建结果字典，然后用 json.dumps 序列化，避免手动拼接错误
            result_dict = {}
            id_start = 1
            ...
            for i, item in enumerate(entries):
                ...
                if entry_dict:  # 只添加非空条目
                    result_dict[str(current_id)] = entry_dict
            
            # 使用 json.dumps 生成格式化的 JSON 字符串
